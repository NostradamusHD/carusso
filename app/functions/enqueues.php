...
add_action( 'wp_enqueue_scripts', function () use ($config) {
    ...
    register_assets('script', [
        'handle'    => 'pandoramarketing/js/slideshow-shop',
        'src'       => $config['resources']['script_slideshow_shop'],
        'deps'      => [ ],
        'ver'       => file_version('0.0.1'),
        'in_footer' => true
    ]);
     ...
     *
     */
    if ( is_front_page() || is_page('hombre') || is_page('mujer') ) {
    ...
    if( is_page('contacto') || is_shop() || is_product_category() ) {

        /**
         * --------------------------------------------------------------------------
         * Register Slider HeadBand Contact
         * --------------------------------------------------------------------------
         */
        ...
        register_assets('script', [
            'handle'    => 'pandoramarketing/js/sliders-contact',
            'src'       => $config['resources']['script_slider_contact'],
            'deps'      => [ ],
            'ver'       => file_version('0.0.1'),
            'in_footer' => true
        ]);
